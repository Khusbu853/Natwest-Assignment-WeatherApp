...
function App() {
  ...
  const [recentSearches, setRecentSearches] = useState(() => {
    const storedRecentSearches = sessionStorage.getItem("recentSearches");
  ...
  const handleInputChange = (e) => {
    ...
    setShowRecentSearches(input.length > 0);
  };
        ...
        <div className="recent-searches ">
          <ul>
            {recentSearches.map((recentCity) => (
              ...
              <li key={recentCity} onClick={() => handleRecentSearchClick(recentCity)}>
                {recentCity}
              </li>
            ))}
          </ul>
